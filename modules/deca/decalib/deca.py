...
class DECA(nn.Module):
    ...
    # @torch.no_grad()
    def decode(self, codedict, rendering=True, iddict=None, vis_lmk=False, return_vis=True, use_detail=False,
    ...
    def forward(self, source_pic, driving_pic):
        bs = source_pic.size(0)
        downsampling = Transforms.Resize((224, 224))
        source_pic = downsampling(source_pic)
        driving_pic = downsampling(driving_pic)
        source_codedict = self.encode(source_pic)

        driving_codedict = self.encode(driving_pic)
        out_list = []
        source_keys = ['shape', 'tex', 'cam', 'light']
        driving_keys = ['exp', 'pose']
        for key in driving_codedict.keys():
            if key in source_keys:
                out_list.append(source_codedict[key].view(bs, -1))
            elif key in driving_keys:
                out_list.append(driving_codedict[key].view(bs, -1))
        out_tensor = torch.cat(out_list, dim=1)

        return out_tensor, source_codedict, driving_codedict   # bs * 236
